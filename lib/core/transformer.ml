...
let predicate_for_exists goal =
  let newgoalname = C.new_predicate_name () in
...
let rec build_tree_deps1 (prefix:hfl list) (path:string list) defs_map (deps:hfl list D.t) args (v:string) : (hfl list * string * hfl list) list =
  ...
  let pred_names_indices : (hfl list * string * hfl list) list =
    List.map (fun pred_call ->
        ...
        let args' = C.substitute_args to_be args a in
        (prefix,p,args')
      ) pred_names_args
  in
...
let get_times_of_unfold orig_formula target_formula =
  ...
  let orig_preds = L.get_predicates orig_formula in
  let target_preds = L.get_predicates target_formula in
...
let get_unfold_path1 defs_map (all_deps : (hfl list * string * hfl list) list D.t) f =
  ...
  let update_path param by ((path, last_pred, last_args) : (hfl list * string * hfl list)) =
    ...
    let path' = List.map (fun node ->
                    ...
                    let args' = C.substitute_args to_be by args in
                    P.pp_list P.pp_formula args' |> P.dbg "args(after)";
                    U.implode_pred pn args'
                  ) path in
    let last_args' = C.substitute_args to_be by last_args in
...
let graphed_unfold_fold1 transformer goal defs_map f =
  let deps : hfl list D.t =
    D.fold (fun v f acc ->
        ...
        let preds = get_predicate_set_ex bd in
...
let get_recursive_groups defs_map f =
  ...
  let f_preds = L.get_predicates f |> List.map U.explode_pred |> List.map fst in
...
let get_perm fvs grp_map ls =
  ...
  let ls'' = List.map (fun (i, (pn, p_args)) -> (i, (pn, List.map (C.subs_vars fvs fvs'') p_args))) ls' in
  let rec aux xs =
    match xs with
    [] -> [[]]
  | x::xs ->
  ...
  let res : (int * (string * hfl list)) list list = (aux ls') in
...
let get_pre_candidates grp_map f =
  let fvs = U.fv f in
  let pr_ns = L.get_predicates f |> List.map U.explode_pred in
...
let get_optimized_constraints defs_map f (deltas : (int * 'a) D.t) =
  ...
  let pred_calls = L.get_predicates f in
  (** n_i, P_i, [t_i] *)
  ...
  let mk_mod_constraints (n_p, pn, args_params) =
    let f5 ((x, c), param) =
      ...
      let nc = H.Var (P.var_to_str n_p ^ x) in
...
let get_general_constrains _ f _ =
  let preds_f = L.get_predicates f in
...
let get_constraints defs_map f (deltas : (int * 'a) D.t) =
  let rec all_args_one_var f =
    ...
    in
    let preds = L.get_predicates f in
...
let is_unfolding_candidate transformer splitter joiner goal defs_map f deltas (src) fvs (dest : (int * (string * hfl list)) list ) =
  ...
  let get_next a args =
    ...
    let pred_calls = pred.H.body |> L.get_predicates in
    if List.length pred_calls = 1 then
      let pred_call = List.hd pred_calls in
      let pred_call' = C.subs_vars pred.H.args args pred_call in
  ...
  let make_constraint (n,(_,(src,org_arg)),(_,(dest,args'')), dist, mid_args) =
      ...
      List.map (fun (arg'', mid_arg, unit_delta) ->
          ...
          let mod_constraint =
            if List.length arg_fv = 1 then
              ...
              let nc = H.Var (P.var_to_str n ^ x) in
              let a2 = H.mk_op Arith.Mult [nc; c] in  
    ...
    P.pp_list P.pp_formula constraints |> P.dbg "constraints";
    (((P.var_to_str n, (dist, l)), (n, (src, org_arg))), (constraints, List.concat mod_constraints)) 
  in
  let sorter ord xs =
  ...
  try
    ...
    P.pp_formula f'' |> P.dbg "f''";
    let goal = {H.var=goal.var; args=goal.args; fix=goal.fix; body=goal_body} in 
    let is_matched, f' = C.fold goal f'' in
    if is_matched then
      true, Some f'
    else
      false, None
...
let guess_based_unfold_fold transformer splitter joiner goal defs_map f =
  ...
  | Some f'' ->
     let fv_f'' = U.fv f'' in
     let params = List.filter (fun v -> List.mem v fv_f'') goal.H.args in
     let newpred = C.mk_rule goal.var params goal.fix f'' in
...
let mult_uf transformer splitter joiner goal defs_map f =
  let units = splitter f in
  let preds, rest = List.partition C.is_pred units in 
  let folded_formulas = Mult_transformer.mult_unfold_fold transformer splitter goal defs_map preds in
  let f' = joiner (folded_formulas @ rest) in

  if Mult_transformer.is_formula_folded goal.H.var f' then
    begin
     let fv_f' = U.fv f' in
     let params = List.filter (fun v -> List.mem v fv_f') goal.H.args in
     let newpred = C.mk_rule goal.var params goal.fix f' in
...
and step2 qvs f defs_map p_a env predname =
  ...
  let goalpred = C.mk_rule predname ps Fixpoint.Least body' in
  
  let transformer f =
    f
    |> normalize_exists qvs
  in
  P.pp "Unfold fold from Existential\n";
...
and ex_dist2 qvs f =
  let rec aux s = function
    ...
    | H.And _ as f ->
       ...
       let fs = f' |> C.cnf_of_formula |> U.get_conjuncts in (** Optimization Point  *)
       ...
       let is_in' = C.mk_ands is_ins' in
       let not_in' = C.mk_ands not_ins in
       let f' = C.mk_ands (not_ins @ is_ins') in
       f'
    | H.Exists (s1, f) ->
       begin
         let f' = aux s f in
...
let unfold_fold_common transformer goal defs_map f splitter joiner =
  ...
  let goal' = C.mk_rule (goal.H.var) goal.H.args goal.H.fix goalbody' in
  P.pp_rule goal' |> P.dbg "RULE'";
  P.pp "Unfold fold from Common\n";
...
let rec unfold_fold_conj goal defs_map f =
  let transformer f = f |> C.dnf_of_formula |> unsat_elim |> taut_elim |> reduce_disj in
  unfold_fold_common transformer goal defs_map f U.get_conjuncts H.mk_ands
;;

let rec unfold_fold_disj goal defs_map f =
  let transformer f = f |> C.cnf_of_formula |> unsat_elim |> taut_elim |> reduce_conj in
...
let rec transform_disjunction goal defs_map env f =
  ...
  let new_preds, body =
    ...
    | _ -> None, H.Bool false
  in
  let atomic = C.mk_ors atomics in
  let f' =
...
and transform_conjunction goal defs_map env f =
  ...
  let f' =
    if List.length atomics = 0 then
      ...
      let f' = C.mk_ands [atomic; body] in
      f'
  in
  (concat_option new_preds preds, f')
